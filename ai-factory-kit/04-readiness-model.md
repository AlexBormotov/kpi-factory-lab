...
2. **Agent-driven pass.** Drop this document into the repository and ask an agent to score the
   repository against `04-readiness-model.md`, producing a filled checklist in which every answer
   cites its evidence: a file path, a command and its output. Then review and adjust it yourself.
   An unreviewed agent-produced score is a claim, not an assessment.
...
| Level | Posture                                                                              |
| ----- | ------------------------------------------------------------------------------------ |
| L1    | No deterministic correctness validator; mistakes are caught informally in review.     |
| L2    | A validator is configured and documented, but nothing enforces it.                    |
| L3    | Validation runs on **every path** by which code reaches the protected branch.         |
| L4    | Pre-commit hooks block bad changes locally; type coverage > 90%.                      |
| L5    | Project-specific rules (custom lints, project-aware codemods, domain guards).         |

**What counts as a validator (revised 2026-08-02).** This pillar measures whether *defects* are
caught mechanically, not whether a particular tool is installed. A strict compiler or type
checker is a stronger validator than a style linter, and a repo that has one is not
"Pre-Functional". Any of these satisfies L1 for its language: a compiler in a compiled language,
a type checker (`tsc --noEmit` with `strict`, mypy, Pyright), or a defect-focused linter (Ruff's
F rules, ESLint's correctness rules, `go vet`). If a linter is redundant because the compiler
already covers those defect classes, record a documented **N/A** rather than a fail.

**Formatting is diff hygiene, not readiness.** A missing formatter makes diffs noisier; it does
not make defects more likely. It is scored as a separate item below and never blocks a level on
its own.

**Checklist:**

* [ ] **L1** A deterministic correctness validator exists for each primary language (compiler,
  type checker, or defect-focused linter). Documented N/A is allowed with a written reason.
* [ ] **L2** Validator config is committed to the repo.
* [ ] **L2** Documentation tells an agent how to run it (`make lint`, `pnpm typecheck`, etc.).
* [ ] **L3** The validator runs automatically and fails on violations.
* [ ] **L3** It runs on **every path into the protected branch** - required status checks, and
  direct pushes either blocked or subject to the same check. A workflow that merely exists, on a
  branch anyone can push to, is a detector and does **not** satisfy this item.
* [ ] **L3** A type checker is configured if the language supports it and is not already the
  validator above.
* [ ] **L4** Pre-commit hook runs the validator locally.
* [ ] **L4** Type strictness is high (TS `strict: true`, mypy `strict`, no broad `any`).
* [ ] **L5** Repo-specific rules encode architectural or domain constraints (layering, imports,
  naming, or a project-authored guard such as "no secrets in a URL" or "no non-Latin text in a
  user-visible string"). A hand-written deterministic guard counts here in full.

**Diff hygiene (scored separately, does not gate a level):**

* [ ] A formatter exists and its config is committed.
* [ ] Formatting is checked automatically.

**Why this was revised.** The previous wording made "a linter exists" and "a formatter exists"
two hard L1 gates. Applied literally, a repository with `tsc --noEmit`, `strict: true` enforced
in CI, and a bespoke deterministic guard scored **Pre-Functional** for having no ESLint, while a
repository with a configured-but-never-run linter scored higher. That inverts the thing the
pillar is supposed to measure. The rule now follows the defect, not the tool.
...
* [ ] **L2** The protected ref cannot accept a commit that has not passed the required checks.
  Written this way on purpose, because the older wording ("branch protection prevents direct pushes
  to the main branch") assumes one branching model and makes trunk-based development unscoreable:
  a repository that pushes to trunk by design could never reach L2 here, and therefore could never
  reach the L3 that gate G2 requires, while the method it is scored against supports trunk-based
  explicitly. Under a review-gated flow this item is satisfied by branch protection with required
  status checks. Under trunk-based development it is satisfied by a server-side check bound to the
  exact SHA, or by protected post-push checks with a stop-line and revert. What is being scored is
  whether the ref can accept uncertified code, not which branching model produced it.
...
* [ ] **L3** The agent context file documents a plan-mode triggering policy: when the agent must plan before acting (non-trivial tasks, architectural decisions, multi-step changes). See `skills/cherny-workflow/SKILL.md`, rule 1.
* [ ] **L3** A lessons-capture convention is documented: after a user correction, the agent updates a persistent lessons file (`tasks/lessons.md`, `memory/lessons_*.md`, or equivalent) with the pattern and a rule preventing recurrence. See `skills/cherny-workflow/SKILL.md`, rule 3.
...
* [ ] **L4** An explicit autonomous-fix policy is documented: what classes of change the agent may fix without human approval (lint, format, dependency bumps, failing tests, etc.) and what always requires approval (schema migrations, customer-facing copy, anything destructive). See `skills/cherny-workflow/SKILL.md`, rule 6.
* [ ] **L4** A plan stress-testing skill is available for non-trivial decisions (interview about each branch of the decision tree with recommended answers). Example: the `grilling` rule set in `skills/`. Use before committing to architectural choices or multi-step plans.
...
9. Add a plan-mode triggering policy and a lessons-capture convention to `AGENTS.md` / `CLAUDE.md`. The off-the-shelf default is in `skills/cherny-workflow/SKILL.md`.
...
* `skills/cherny-workflow/SKILL.md`: six in-session workflow rules (plan-mode default, subagent
  strategy, self-improvement loop, verification before done, demand elegance, autonomous bug fixing),
  derived from Boris Cherny's published engineering guide. Connects to Pillar 10, and partly to
  Pillars 3, 4, 7 and 11.
* `skills/`: the five rule sets this kit installs. See `INSTALL.md`.
  * `agentic-factory`: the stream-level method, the nine gates and the vocabulary boundary.
  * `cherny-workflow`: the session-level rules described above.
  * `simplicity-ladder`: stop at the first rung that solves the problem, inside the standing
    architectural decision.
  * `boundary-dispatch-matrix`: one row per dispatch path before execution crosses a boundary.
  * `grilling`: stress-test a plan by relentless interview down the decision tree.
